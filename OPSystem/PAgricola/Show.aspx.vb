...
Public Class Show
            ...
            Select Case oRp.Reporte
                Case "SALIDASEMILLA"
                    ...
                    If Not IsPostBack Then
                        ...
                        customerReport.ExportToDisk(ExportFormatType.PortableDocFormat, Server.MapPath("~/") + "App_Data\SALIDAS SEMILLA\" + oRp.Nombre)
                        'SendMail()
                        'SendMailGrl("SE HA GENERADO UNA NUEVA ORDEN DE FLETE, REVISAR ARCHIVO ADJUNTO",
                        '"_MAILFROM", "fletesajo@grupou.mx", "8149pfegord$", "mail.grupou.mx", 587, "_MAILTO")
                        Session.Remove("INFORP")
                        'Transferir_Archivo(oUsr.Mis.Log.Replace("Err.log", oRp.Nombre), oRp.Nombre)
                        Transferir_Archivo(Server.MapPath("~/") + "App_Data\SALIDAS SEMILLA\" + oRp.Nombre, oRp.Nombre)
                    End If
                ...
                Case "FLETE_SEMILLA"
                    ...
                    If Not IsPostBack Then
                        'customerReport.ExportToDisk(ExportFormatType.PortableDocFormat, oUsr.Mis.Log.Replace("Err.log", oRp.Nombre))
                        customerReport.ExportToDisk(ExportFormatType.PortableDocFormat, Server.MapPath("~/") + "App_Data\FLETES\" + oRp.Nombre)
                        'SendMail()
                        If oRp.Envia = "S" Then
                            SendMailGrl(oRp.Descripcion,
                                    "_MAILFROM", "fletesajo@grupou.mx", "8149pfegord$", "mail.grupou.mx", 587, "_MAILFP", Server.MapPath("~/") + "App_Data\FLETES\" + oRp.Nombre)
                        End If
                        Session.Remove("INFORP")
                        'Transferir_Archivo(oUsr.Mis.Log.Replace("Err.log", oRp.Nombre), oRp.Nombre)
                        Transferir_Archivo(Server.MapPath("~/") + "App_Data\FLETES\" + oRp.Nombre, oRp.Nombre)
                    End If
    ...
    Public Function SendMail() As Boolean
        ...
        Try
            With oInf
                ...
                Dim ArTo As List(Of String) = oRp.Parametros.ItemValue("_MAILFP")
    ...
    Public Function SendMailGrl(
            ByRef body As String, ByRef MailFrom As String,
            ByRef mail As String, ByRef passw As String,
            ByRef Host As String, ByRef puerto As Integer,
            ByRef Destinatarios As String, ByRef Adj As String
            ) As Boolean
        Dim oSql As New SQLSetting(oUsr)
        Dim oCs As New ColeccionPrmSql
        Dim oInf As New InfoMail
        ...
        Try
            With oInf
                ...
                .MailAttach.Add(Adj)
                .MailBody = body
                .MailFrom = oRp.Parametros.ItemValue(MailFrom)
                .MailName = oRp.Nombre
                .MailSmtpCredenciales.Create("@username", mail)
                .MailSmtpCredenciales.Create("@password", passw)
                .MailSmtpEnableSsl = False
                .MailSmtpHost = Host
                .MailSmtpPort = puerto
                Dim ArTo As List(Of String) = oRp.Parametros.ItemValue(Destinatarios)
                For Each sTo As String In ArTo
                    .MailTo.Add(sTo)
                Next
            End With
